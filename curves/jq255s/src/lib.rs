...
//!
//! Curve information:
//! * Base field: q = 2^{255} - 3957
//!                 = 57896044618658097711785492504343953926634992332820282019728792003956564816011
//! * Scalar field: 2r = 2(2^{254}+56904135270672826811114353017034461895)
...
mod tests {
    ...
    fn valid_point_addition() {
        ...
        for encoded_points in encoded_points_vector.iter() {
            let p1 = Affine::deserialize_compressed_unchecked(&*hex::decode(encoded_points[0]).unwrap()).unwrap();
            let p2 = Affine::deserialize_compressed_unchecked(&*hex::decode(encoded_points[1]).unwrap()).unwrap();

            let p1_p2 = p1 + (p2 + Projective::n());
            let p3 = Affine::deserialize_compressed_unchecked(&*hex::decode(encoded_points[2]).unwrap()).unwrap().into_group();
            assert!(p1_p2 == (p3 + Projective::n()));

            let p1_p1 = p1 + p1;
            let p4 = Affine::deserialize_compressed_unchecked(&*hex::decode(encoded_points[3]).unwrap()).unwrap().into_group();
            assert!(p1_p1 == p4);

            let p1_p1_p2 = p1 + p1 + p2;
            let p5 = Affine::deserialize_compressed_unchecked(&*hex::decode(encoded_points[4]).unwrap()).unwrap().into_group();
            assert!(p1_p1_p2 == p5);

            let p1_p1_p2_p2 = p1 + p1 + p2 + p2;
            let p6 = Affine::deserialize_compressed_unchecked(&*hex::decode(encoded_points[5]).unwrap()).unwrap().into_group();
    ...
    fn valid_scalar_multiplication() {
        ...
        for encoded in encoded_points_and_scalars_vec.iter() {
            let p = Affine::deserialize_compressed_unchecked(&*hex::decode(encoded[0]).unwrap()).unwrap().into_group();
            let n = crate::Fr::from(bytes_to_bigint(hex::decode(encoded[1]).unwrap()));
            let pn = p * n;
            let p2 = Affine::deserialize_compressed_unchecked(&*hex::decode(encoded[2]).unwrap()).unwrap().into_group();
            assert!(pn == p2)
        }
    }
}
