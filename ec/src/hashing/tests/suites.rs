...
use crate::hashing::tests::DefaultHasher;
...
fn suites() {
    ...
    fn tt(u: &SuiteVector) -> Outcome {
        ...
        match u.curve.as_str() {
            "BLS12-381 G1" => {
                hasher = <DefaultHasher<Sha256, 128> as HashToField<Fq>>::new_hash_to_field(dst)
                    .unwrap();
            },
            "BLS12-381 G2" => {
                hasher = <DefaultHasher<Sha256, 128> as HashToField<Fq2>>::new_hash_to_field(dst)
                    .unwrap();
            },
