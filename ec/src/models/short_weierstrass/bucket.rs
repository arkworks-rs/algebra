...
impl<'a, P: SWCurveConfig> AddAssign<&'a Bucket<P>> for Projective<P> {
    ...
    /// (XYZZ) coordinates, which stores Z^2 and Z^3 separately to avoid redundant
    /// computations.
    ///
    /// In Extended Jacobian (Bucket) coordinates:
    /// - x = X/ZZ, where ZZ = Z^2, so x = X / Z^2
    /// - y = Y/ZZZ, where ZZZ = Z^3, so y = Y / Z^3
    /// - zz = Z^2
    /// - zzz = Z^3
    ///
    /// The key optimization is that we can compute:
    /// - U1 = X1 * Z2^2 = x1 * z1^2 * zz2 (since Z2^2 = zz2)
    /// - U2 = X2 * Z1^2 = x2 * zz2 * z1^2 (since X2 = x2 * zz2)
    /// - S1 = Y1 * Z2^3 = y1 * z1^3 * zzz2 (since Z2^3 = zzz2)
    /// - S2 = Y2 * Z1^3 = y2 * zzz2 * z1^3 (since Y2 = y2 * zzz2)
    ///
    /// This avoids computing Z2^2 and Z2^3 from Z2, since they are already
    ...
    fn add_assign(&mut self, other: &'a Bucket<P>) {
        ...
        // Z2Z2 = Z2^2 (already stored in Bucket as zz)
        let z2z2 = other.zz;

        // U1 = X1 * Z2^2
        // Since x1 = X1/z1^2, we have X1 = x1 * z1^2
        // So U1 = x1 * z1^2 * Z2^2 = x1 * z1^2 * z2z2
        let mut u1 = self.x;
        u1 *= &z1z1;
        u1 *= &z2z2;
        ...
        // Z2Z2Z2 = Z2^3 (already stored in Bucket as zzz)
        let z2z2z2 = other.zzz;

        // S1 = Y1 * Z2^3
        // Since y1 = Y1/z1^3, we have Y1 = y1 * z1^3
        // So S1 = y1 * z1^3 * Z2^3 = y1 * z1^3 * z2z2z2
        let mut s1 = self.y;
        s1 *= &self.z;
        s1 *= &z1z1;
        s1 *= &z2z2z2;
        ...
        // S2 = Y2 * Z1^3
        // In Bucket coordinates, y2 = Y2/zzz2, so Y2 = y2 * zzz2
        ...
        if u1 == u2 {
            ...
            // Since zzz2 = Z2^3 and zz2 = Z2^2, we have Z2 = zzz2/zz2
            // So: Z3 = 2 * z1 * (zzz2/zz2) * H = 2 * z1 * zzz2 * H / zz2
            self.z *= &other.zzz;
            self.z.double_in_place();
            self.z *= &h;
            let z2z2_inv = other.zz.inverse().unwrap();
            self.z *= &z2z2_inv;
        }
    }
}
