    ...
    fn deserialize_with_mode<R: Read>(
        ...
        let (e, u) = match compress {
            Compress::Yes => {
                ...
                let e: Self::BaseField = (Self::get_c() * u.square().square() - (Self::COEFF_A * u.square()).double() + Self::BaseField::ONE)
                    .sqrt().ok_or(SerializationError::InvalidData).unwrap();
                (e, u)
            },
            Compress::No => {
                let e: Self::BaseField = CanonicalDeserialize::deserialize_with_mode(&mut reader, compress, validate)?;
                let u: Self::BaseField = CanonicalDeserialize::deserialize_with_mode(&mut reader, compress, validate)?;
                (e, u)
            }
        };
